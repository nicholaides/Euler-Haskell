...
withoutDiagonal :: Int -> Grid -> Grid
withoutDiagonal n grid = map (\(m,row) -> if m < n then tail row else row ) $ zip [0..] $ filter (/= []) grid

diagonals :: Grid -> Grid
diagonals grid = diagonals' 1 [] grid

diagonals' :: Int -> Grid -> Grid -> Grid
diagonals' _ diags [[]] = diags
diagonals' n diags grid = let
    diag = diagonal n grid
    newDiags = diags ++ [diag]
    newGrid  = withoutDiagonal n grid
  in
